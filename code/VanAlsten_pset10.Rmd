...
dat.dm <- dat.dm %>%
  #need to demean the actual squared term; not just square the demeaned base
  #hence why squared term defined apart from I() syntax
  group_by(nr) %>%
  select(-year)%>%
...
All point estimates are the same, as are the number of observations, R^2 values and the model F-test.
