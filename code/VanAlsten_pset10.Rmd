...
#print out results
stargazer::stargazer(mod.tf, float = FALSE)
...
#include linear time effect in model
mod.time.trend <- plm(lwage ~ educ + black + hisp + exper +
                      exper_sq + married + union + year_num,
                      data = pdata,
                      effect = "individual",
                      model = "within")
summary(mod.time.trend)

lmtest::coeftest(mod.time.trend, vcov=function(x) vcovHC(x, cluster="group", type="HC1"))
