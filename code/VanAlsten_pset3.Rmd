...
treatFxClus <- function(v1, v0, cluster){
  ...
  treat <- sample(x = cluster, size = length(unique(cluster))/2, replace = FALSE)
  ...
  #outcomes if treated, otherwise not observed
   ...
   #because clusters are equal size, can use standard robust
   #estimator from diff in means
   #(var of treat group/n treat group) + (var control/n control)
  ate.var <- (var(olken[olken$treat.invite == 1, "pct.missing"])/
              nrow(olken[olken$treat.invite == 1,])) +
  (var(olken[olken$treat.invite == 0, "pct.missing"])/
     nrow(olken[olken$treat.invite == 0,]))
