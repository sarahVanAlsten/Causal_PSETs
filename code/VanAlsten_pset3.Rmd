...
treatFxClus <- function(v1, v0, cluster){
  ...
  #create vector for obs outcomes under treatment: keep 
  #outcomes if treated, otherwise not observed
   yi1 <- ifelse(cluster %in% treat, v1, NA)
   
   #create vector for obs outcomes under control: keep 
   #outcomes if control (!treat), otherwise not observed
   yi0 <- ifelse(!cluster %in% treat, v0, NA)
   
   
   #####################################
   # Get clusters that weren't treated
   #####################################
     
  #control is every other cluster
  #create index of clusters that have not been treated
  control.index <- ! 1:50 %in% treat
  
  #initialize dummy vector for control of length 25
  control <- c(rep(999, 25))
  
  #iterate over it to add control group #s
  #j = index of vector to add to, start it at 1
  
  j <- 1
  for (i in 1:50){
    if (control.index[i]){
      control[j] <- i
      j <- j + 1 #add 1 to move index up
    }
  }
   
  #now 'control' is the list of clusters not treated
 
  ##########################################
   # Get ATE
   ########################################
   
   #clusters are equal size, so simple diff in means is unbiased
   #estimator of ATE
   ate <- mean(yi1, na.rm = T) - mean(yi0, na.rm = T)
  ...
  #get p value
  test.res <-
    Hmisc::t.test.cluster(y = c(yi1[!is.na(yi1)], yi0[!is.na(yi0)]),
                          cluster = cluster, group =  c(rep(1,500), rep(0,500)))
...
```{r}
  ...
  #################################
  #Make a data frame for the outcom
  ...
  #get variance by cluster: pt 2 (actually getting variance)
  #means for treatment clusters
  treat.means <- aggregate(cbind(yi1[!is.na(yi1)]), by = list(treat = rep(treat, each = 20)), 
                           FUN = mean, na.rm = T)
  
  #mean for control clusters
  control.means <- aggregate(cbind(yi0[!is.na(yi0)]), by = list(control = rep(control, each = 20)),
                             FUN = mean, na.rm = T)
   
  #get global mean (of all observations)
  global.mean <- mean(c(yi1, yi0), na.rm = T)
   
  #get total variance: sum of all individual Y - global mean
  tot.var <- sum((c(yi1[!is.na(yi1)], yi0[!is.na(yi0)]) - global.mean)^2)
  
  #get betwen cluster variance (squared deviations of group means - global mean)
  between.var <- sum(20*((c(treat.means$V1, control.means$V1) - global.mean)^2))
  ...
  #standard error:sqrt(var yi1/n yi1 + var yi0/n yi0)
  se <- sqrt((var(yi1, na.rm = T)/500) +
               var(yi0, na.rm = T)/500)
  
  
  #multiply by design correction
  se.final <- se*sqrt(design.correction)
  



test.res <-
    Hmisc::t.test.cluster(y = c(yi1, yi0), cluster = c(treat, control), group =  c(rep(1,25), rep(0,25)))
  ...
  #get variance by cluster: pt 2 (actually getting variance)
  #means for treatment clusters
  treat.means <- aggregate(cbind(yi1[!is.na(yi1)]), by = list(treat = treat), FUN = mean, na.rm = T)
  
  #mean for control clusters
  control.means <- aggregate(cbind(yi0[!is.na(yi0)]), by = list(control = control), FUN = mean, na.rm = T)
